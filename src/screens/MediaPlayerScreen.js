...
export default function MediaPlayerScreen({ route, navigation }) {
  ...
  const handleDownload = async () => {
    if (!isAuthenticated) {
      Alert.alert('Login Required', 'Please login to download')
      return
    }
    try {
      ...
      await downloadService.complete({
        ...
        storageKey: `nendplay_media_${mediaId}`,
        storedFileSize: media?.fileSize || 0,
      })
      Alert.alert('Downloaded', 'Added to your downloads!')
    } catch (err) {
      Alert.alert('Download Failed', err.response?.data?.message || 'Please subscribe to download')
    }
  }
  ...
  const streamUrl = mediaService.getStreamUrl(mediaId)
  ...
  const played = status.positionMillis || 0
